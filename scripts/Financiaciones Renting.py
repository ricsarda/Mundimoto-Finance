...
# === Función auxiliar para convertir valores numéricos con separadores de miles
...
def extraer_financiaciones(texto, lineas, nombre_pdf):
    ...
    info_pdf = {
        ...
        'Operación': None,
        'Archivo': nombre_pdf  # Guardar el nombre del archivo sin la extensión
    }
...
def main(files, pdfs=None, new_excel=None, month=None, year=None):
    try:
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            ws_amort = writer.book.add_worksheet("Amortizaciones")
            row_offset = 0

            for pdf_name, info_amort in amortizaciones_por_pdf:
                df_amort = info_amort['df']
                codigo = info_amort['codigo'] or "COD NO ENCONTRADO"
                fecha_recal = info_amort['fecha_recal'] or "FECHA NO ENCONTRADA"

                ws_amort.write(row_offset, 0, codigo)
                ws_amort.write(row_offset + 1, 0, fecha_recal)
                ...
                row_offset += df_amort.shape[0] + 10
